...
double v;
   ...
   case FAM_SUM:
      ...
      break;
   case FAM_ZDR:
      /* The scale wraps; zdr_value() in palette.c is the one copy of it,
       * shared with the cross section and the hole filling, because the
       * colour comes from a palette indexed by the byte - a reader that
       * wraps it differently prints one value and paints another. */
      v=zdr_value(value);
      sprintf(text,"= %.1f ¤",v);
      break;
   case FAM_VEL:
