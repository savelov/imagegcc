...
# selection was ignored and one raster was written seven times.
echo "=== all seven differ"
if command -v md5 >/dev/null 2>&1; then HASH="md5 -q"; else HASH=md5sum; fi
n=$(ls "$OUT"/*.tif 2>/dev/null | wc -l)
u=$($HASH "$OUT"/*.tif 2>/dev/null | awk '{print $1}' | sort -u | wc -l)
echo "    $n files, $u distinct"
[ "$n" -eq 7 ] || { echo "    FAIL: expected 7 files"; rc=1; }
